...
# =============================================================================
"""Contains script to train deep k-learning architecture (generative) for experimental data."""
        ...
        for alpha in alphas:
            ...
            if pretrain is True:
                ...
                test_output = generator.predict(test_input,
                             ...
                             verbose=1)
                #save the model
                generator.save(prefix + 'exp-generative-pretrained-R-' + str(R) + '-phperpix-' + str(photon_level) + '-alpha-' + str(alpha) + '.h5')
                ...
                generator.save(prefix + 'exp-generative-not-pretrained-R-' + str(R) + '-phperpix-' + str(photon_level) + '-alpha-' + str(alpha) + '.h5')
            ...
            # save data
            if pretrain is True:
                io.savemat(date_string + '-exp-pretrained-alpha-%0.2f-R-%0.2f.mat' % (alpha, R),
                ...
                io.savemat(date_string + '-exp-not-pretrained-alpha-%0.2f-R-%0.2f.mat' % (alpha, R),
                           {'gen_loss_list': np.array(gen_loss_list),
                            'supervised_loss_list': np.array(supervised_loss_list),
                            'val_loss_list': np.array(val_loss_list),
                            'disc_loss_list': np.array(disc_loss_list),
