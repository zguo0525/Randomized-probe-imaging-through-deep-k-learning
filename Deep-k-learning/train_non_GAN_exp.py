...
# =============================================================================
"""Contains script to train deep k-learning architecture (non generative) with experimental data."""
...
if __name__ == "__main__":
    ...
    for photon_level in photon_levels:
        ...
        data_input = np.load('../train-approx-%d-iter-1-lr-1.00.npy' % photon_level)
        # set edges zero
        data_input[:, Rs>128] = 0
        ...
        test_input = np.load('../test-approx-%d-iter-1-lr-1.00.npy' % photon_level)
        # set edges zero
        test_input[:,Rs>128] = 0
        ...
        if pretrain is True:
            ...
            model.save(prefix + 'exp-pretrained-R-' + str(R) + '-peak-' + str(photon_level) + '.h5')

            io.savemat(date_string + 'exp-pretrained-R-' + str(R) + '-peak-' + str(photon_level) + '.mat',{'train_loss1':np.array(train_loss1),
            ...
            model.save(prefix + 'exp-not-pretrained-R-' + str(R) + '-peak-' + str(photon_level) + '.h5')

            io.savemat(date_string + 'exp-not-pretrained-R-' + str(R) + '-peak-' + str(photon_level) + '.mat',{'train_loss':np.array(train_loss1),
